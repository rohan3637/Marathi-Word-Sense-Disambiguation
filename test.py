...
health_word_index = pickle.load(open("Health_Model\word_index.pkl", "rb"))
health_pos_index = pickle.load(open("Health_Model\pos_index.pkl", "rb"))

tourism_word_index = pickle.load(open("Tourism_Model\word_index.pkl", "rb"))
tourism_pos_index = pickle.load(open("Tourism_Model\pos_index.pkl", "rb"))

news_word_index = pickle.load(open("News_Model\\news_word_index.pkl", "rb"))
news_pos_index = pickle.load(open("News_Model\\news_tag_index.pkl", "rb"))

nouns = pickle.load(open("WordSenseDisambiguationModel\\nouns.pkl", "rb"))
...
def preprocess_news_input_text(text, MAXLEN=129,word_index=news_word_index):
    ...
    for word in sentence:
        try:
            ...
            sentence_numeric.append(word_index["_OOV_"])
    sentence_numeric = pad_sequences([sentence_numeric], maxlen=MAXLEN, padding="pre",truncating="post")
    return sentence_numeric.reshape((MAXLEN,))
...
health_json_file = open('Health_Model\health_model.json', 'r')
...
health_model.load_weights("Health_Model\health_model_weights.h5")

tourism_json_file = open('Tourism_Model\\tourism_model.json', 'r')
...
tourism_model.load_weights("Tourism_Model\\tourism_model_weights.h5")

news_json_file = open('News_Model\\news_model.json', 'r')
news_loaded_model_json = news_json_file.read()
news_json_file.close()
news_model = model_from_json(news_loaded_model_json)
news_model.load_weights("News_Model\\news_model_weights.h5")


red_text = "<span style=\" font-size:15px; font-weight:600; color:red;\" >{}</span> "
green_text = "<span style=\" font-size:15px; font-weight:600; color:green;\" >{}</span> "
yellow_text = "<span style=\" font-size:15px; font-weight:600; color:orange;\" >{}</span> "
...
class Screen2(QMainWindow):
    ...
    def initUI(self):
        ...
        domainList = ["Tourism", "Health", "News"]
    ...
    def helperRedirect(self):
        ...
        if len(content) != 0:
            if current_combo_text == "Health":
                ...
                preds = predictions[len(predictions)-len(content.split()):]
                pos = []
                for i in preds:
            ...
            for noun in all_nouns:
                senses = self.getSense(stemmer.stemWord(noun))

                # wordsense_output = wordsense_output + red_text.format("Noun: "+str(noun)) + red_text.format(f"stem: {stemmer.stemWord(noun)}") + "<br>"

                if senses:

                    for sense in senses:
                        wordsense_output = wordsense_output + red_text.format(str(noun)) + ": " + str(black_text.format(sense)) + "<br>"


                    # wordsense_output  = wordsense_output + red_text.format(str(noun)) + ": " + str(sense) + "<br>"
                    screen3_nouns.append(noun)
            
            self.screen3 = screen3.Screen3(content, screen3_nouns)
    ...
    def getSense(self, noun):
        ...
        try:
            ...
            #         end = "."
            #     if ":" in meanings[syn_ids[i]]:
            #         all_senses = all_senses + meanings[syn_ids[i]].split(":")[0] + end
            #     else:
            #         all_senses = all_senses + meanings[syn_ids[i]].split(".")[0] + end
            # return black_text.format(all_senses)
            return all_senses
